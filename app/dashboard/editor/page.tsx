...
export default function EditorPage() {
  ...
  // Single ref for the entire right toolbar (used for both chunk + clean outside-click)
  ...
  // Close chunk/clean popovers on outside click (single handler for the whole toolbar)
  useEffect(() => {
    ...
    <div className="flex flex-col" style={{ height: "100vh" }}>
      ...
      >
        ...
        {/*
          ── Right toolbar ──────────────────────────────────────────────────────
          Two-level layout:
            • Outer div  → position:relative (anchors popover panels) + toolbarRef
            • Inner div  → overflow-x:auto scrollable row of trigger buttons only
            • Popover panels are rendered OUTSIDE the inner scroll div so they
              are never clipped by overflow
        */}
        <div ref={toolbarRef} className="relative shrink-0 ml-3">

          {/* ── Scrollable button row ── */}
          <div className="flex items-center gap-2" style={{ overflowX: "auto", scrollbarWidth: "none" }}>

            {/* Chunk trigger */}
            <button type="button" onClick={() => { setChunkOpen((o) => !o); setCleanOpen(false); }}
            ...
            {/* Clean trigger */}
            <button type="button" onClick={() => { setCleanOpen((o) => !o); setChunkOpen(false); setCleanPreview(null); }}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all border"
              style={{
          ...
          {/* ── Popover panels (outside scroll div, not clipped by overflow) ── */}

          {chunkOpen && (
            <div className="absolute right-0 top-full mt-2 rounded-xl border border-white/10 shadow-2xl p-4 w-64"
              style={{ background: "linear-gradient(160deg,#1c0b35 0%,#110620 100%)", zIndex: 300 }}>
