...
export default function LanguagePicker({ value, onChange, disabled }: Props) {
  ...
  // Build grouped list
  const grouped = GROUP_ORDER.reduce<Record<Group, Language[]>>(
    (acc, g) => ({ ...acc, [g]: [] }),
    {} as Record<Group, Language[]>
  );
  ...
  const modal = (
    ...
    >
      <div
        ...
        style={{
          background: "linear-gradient(135deg, #1a0a2e 0%, #160822 50%, #0f0518 100%)",
          maxHeight: "80vh",
        }}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-4 pt-4 pb-3" style={{ borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
          ...
          <button
            type="button"
            onClick={closeModal}
            className="w-7 h-7 rounded-lg flex items-center justify-center text-white/40 hover:text-white/80 transition-all"
        ...
        <div className="px-4 py-3" style={{ borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
          <div className="relative">
            ...
            <input
              ...
              className="w-full rounded-xl pl-9 pr-3 py-2 text-sm text-white placeholder-white/30 outline-none transition-all"
  ...
  return (
    <>
      ...
      <button
        ...
        disabled={disabled}
        className="input-field w-full flex items-center gap-2.5 text-left disabled:opacity-50 disabled:cursor-not-allowed"
      ...
      {/* Modal — portalled to document.body to escape any parent stacking context */}
...
function LangCard({
  ...
  return (
    <button
      ...
      style={{
        background: selected ? "rgba(147,51,234,0.3)" : "rgba(255,255,255,0.03)",
        border: selected ? "1px solid rgba(168,85,247,0.6)" : "1px solid rgba(255,255,255,0.06)",
